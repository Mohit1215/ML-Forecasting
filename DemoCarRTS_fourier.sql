...
train_test_forecast<-function(data){
  ...
  Fourier <- data.frame(fourier(data_msts, K = c(K)))
  ...
  regress_form <- detrended~year+week_day+month+week+day_of_month+seas+FourierS1+FourierC1+FourierS2+FourierC2
...
#}
'
					,@input_data_1 = N'SELECT * FROM democarsales;'
...
train_test_forecast<-function(data){
  ...
  data$fourier <- fourier(data_msts, K = c(K))
  for_fourier <- fourier(data_msts, K = c(K), h=forecast_period)
  ...
  trend_fit<-auto.arima(data_trend)
  trend_for<-as.matrix(forecast(trend_fit,12)$mean)
  regress_form <- detrended~year+week_day+month+week+day_of_month
  
    
  #Create data for forecast
  last_date <- as.Date(train[nrow(train),"Time"])
  forecast_start_date <- seq(last_date, by="month", length.out = forecast_period)[2]
  forecasttest <- data.frame(Time = seq(forecast_start_date, by = "month", length.out = 12))
